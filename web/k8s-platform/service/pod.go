...
func (p *pod) toCells(std []corev1.Pod) []DataCell {
	//make 是一个内置函数，用于创建切片、映射和通道等引用类型的数据结构。
	//[]DataCell 表示要创建的切片的类型，即 DataCell 类型的切片。
	//len(std) 是指定切片的长度，它使用了 std 变量的长度作为切片的长度。
...
// 删除pod
func (p *pod) DeletedPod(podName, namespace string) (err error) {
...
// 获取Pod中的容器名
func (p *pod) GetPodContanier(podName, namespace string) (containers []string, err error) {
	//获取pod详情
...
func (p *pod) GetPodLog(containerName, podName, namespace string) (log string, err error) {
	...
	//发起request请求，返回io.ReadCloser类型（等同于respose.body）

...
func (d *deployment) GetDeployments(filterName, namespace string, limit, page int) (deploymentResp *DeploymentsResp, err error) {
	...
	if err != nil {
		logger.Error(errors.New("获取Deployments列表失败，" + err.Error()))
...
func (d *deployment) GetDeployNumPerNp() (DeploysNps []*DeploysNp, err error) {
	...
	for _, namespace := range namespaceList.Items {
		...
		if err != nil {
			logger.Error(errors.New("获取namespace下的deployment列表失败，" + err.Error()))
			return nil, errors.New("获取namespace下的deployment列表失败，" + err.Error())
		}
		DeploysNp := &DeploysNp{
